# Code Files Configuration Reference

Auto-generated documentation for all code files based on configuration.

## Files Overview

| File | Path | Always Included | Technologies | Features |
|------|------|-----------------|--------------|----------|
| [globals.css](./globals_css.md) | `styles/globals.css` | Yes | tailwindcss | - |
| [auth.ts](./auth_ts.md) | `lib/auth.ts` | No | betterAuth, prisma | authentication |
| [auth-client.ts](./auth-client_ts.md) | `lib/auth-client.ts` | No | betterAuth | authentication |
| [auth.util.ts](./auth_util_ts.md) | `lib/auth.util.ts` | No | prisma, betterAuth | authentication |
| [prisma-rls.ts](./prisma-rls_ts.md) | `lib/prisma-rls.ts` | No | prisma | - |
| [prisma-rls-client.ts](./prisma-rls-client_ts.md) | `lib/prisma-rls-client.ts` | No | prisma | - |
| [log.utils.ts](./log_utils_ts.md) | `lib/log.utils.ts` | Yes | - | - |
| [robots-file](./robots-file.md) | `CLAUDE` | Yes | - | - |

## Files by Technology

### tailwindcss
- [globals.css](./globals_css.md)

### betterAuth
- [auth.ts](./auth_ts.md)
- [auth-client.ts](./auth-client_ts.md)
- [auth.util.ts](./auth_util_ts.md)

### prisma
- [auth.ts](./auth_ts.md)
- [auth.util.ts](./auth_util_ts.md)
- [prisma-rls.ts](./prisma-rls_ts.md)
- [prisma-rls-client.ts](./prisma-rls-client_ts.md)

## Always Included
- [globals.css](./globals_css.md)
- [log.utils.ts](./log_utils_ts.md)
- [robots-file](./robots-file.md)
