...
const claudeMarkdownContent = `# CLAUDE.md
...
const Page = () => {
  ...
  return <PageContent lastSegment={lastSegment} />;
};

const PageContent = ({ lastSegment }: { lastSegment?: string }) => {
  const [selectedLLM, setSelectedLLM] = useState<LLMType>("Claude");
  const isPromptConfig = lastSegment === "prompt.config";

  return (
    <div className="flex h-full flex-col">
      {isPromptConfig && (
        <Toolbar selectedLLM={selectedLLM} onLLMChange={setSelectedLLM} />
      )}

      <div className="flex-1 overflow-auto p-6 bg-background">
        {isPromptConfig ? (
          <div className="prose prose-slate max-w-none dark:prose-invert prose-headings:text-foreground prose-p:text-white prose-li:text-white prose-strong:text-foreground prose-code:text-foreground prose-pre:bg-muted prose-pre:text-foreground">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>
              {claudeMarkdownContent}
            </ReactMarkdown>
          </div>
        ) : (
