...
export const generateNDAPDF = (ndaData: NDAFormData): Blob => {
  const doc = new jsPDF();
...
export const downloadNDAPDF = (ndaData: NDAFormData) => {
  const blob = generateNDAPDF(ndaData);
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `NDA_${ndaData.legalEntityName.replace(/\s+/g, "_")}_${ndaData.effectiveDate}.pdf`;
